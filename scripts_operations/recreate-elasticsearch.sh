#!/usr/bin/env bash
# recreate-elasticsearch.sh — Destroy and recreate the elasticsearch data volume.
#
# WARNING: This permanently deletes ALL 
# elasticsearch (knowledge base, embeddings, etc.).
# The schema will be recreated automatically on the next application boot.
#
# Usage: ./scripts_operations/recreate-elasticsearch.sh
...
echo -e "${YELLOW}WARNING: This will permanently destroy the elasticsearch data volume.${NC}"
echo -e "${YELLOW}All knowledge base collections and vectors will be lost.${NC}"
read -r -p "Type 'yes' to continue: " confirm
[[ "$confirm" == "yes" ]] || { echo "Aborted."; exit 1; }

echo -e "\n${YELLOW}[1/3] Stopping elasticsearch container...${NC}"
docker compose down elasticsearch

echo -e "${YELLOW}[2/3] Removing data volume...${NC}"
docker volume rm gsage-ai_elasticsearch_data 2>/dev/null \
  && echo "    Volume removed." \
  || echo "    Volume not found — skipping."

echo -e "${YELLOW}[3/3] Starting elasticsearch container...${NC}"
docker compose up -d elasticsearch

echo -e "\n${GREEN}Done. elasticsearch is starting fresh.${NC}"
