...
--- Mermaid engine
-- gSage patch: when the environment variable
-- ``PANDOC_DIAGRAM_PUPPETEER_CONFIG`` is set (or a ``puppeteer-config.json``
-- file exists in the bundle directory), pass it to ``mmdc`` so headless
-- Chromium runs with ``--no-sandbox`` and other container-friendly flags.
local mermaid = {
  line_comment_start = '%%',
  mime_types = mime_types_set{'pdf', 'png', 'svg'},
  compile = function (self, code)
    ...
    local puppeteer_config = os.getenv('PANDOC_DIAGRAM_PUPPETEER_CONFIG')
    return with_temporary_directory("diagram", function (tmpdir)
      return with_working_directory(tmpdir, function ()
        ...
        if puppeteer_config and puppeteer_config ~= '' then
