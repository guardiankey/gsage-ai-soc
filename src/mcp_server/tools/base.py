...
from src.shared.interaction.exceptions import (
...
class BaseTool(ABC):
    ...
    # ── Interaction Service ────────────────────────────────────────────────
    # Injected by run() before execute() is called.  Tools access it via
    # ``self.interaction`` to request user input (forms, confirmations, …).
    _interaction: Any = None

    @property
    def interaction(self) -> Any:
        """Access the InteractionService from within execute().

        Raises RuntimeError if not yet injected (i.e. called outside execute()).
        """
        if self._interaction is None:
            raise RuntimeError(
                "InteractionService not available. "
                "It is injected by BaseTool.run() before execute() is called."
            )
    ...
    async def run(
        ...
        Args:
            ...
            gsage_session_id: Conversation session for SSE correlation.
            tool_call_id: Unique ID for this tool invocation (generated by
                ``handle_call_tool`` in the MCP server, NEVER by the tool).
        ...
        # ── 4e. Inject InteractionService ────────────────────────────────
        # Available to tools via ``self.interaction`` during execute().
        if gsage_session_id is not None:
            from src.shared.interaction.broker_redis import RedisBroker  # noqa: PLC0415
            from src.shared.interaction.service import InteractionService  # noqa: PLC0415

            broker = RedisBroker(redis_client=redis_client)
            self._interaction = InteractionService(
                ...
                gsage_session_id=gsage_session_id,
                tool_name=self.name,
                execution_id=getattr(agent_context, "request_id", None),
        ...
        try:
            while attempt < max_attempts:
                try:
                ...
                except InteractionReplanRequested as exc:
                    # REPLAN_AGENT — tool deliberately aborted to let the
                    # agent replan with user-supplied data.  Return a
                    # structured result so the agent framework can act on it.
                    elapsed = int((time.monotonic() - start) * 1000)
                    result = ToolResult(
                        status="interaction_replan",
                        data={
                            "interaction_id": str(exc.interaction_id),
                            "schema": exc.schema,
                            "context": exc.context,
                        },
                        error=None,
                    ...
                    # Do not retry — this is a deliberate abort, not a failure.
                    break
