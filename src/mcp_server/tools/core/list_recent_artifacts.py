...
class ListRecentArtifactsTool(BaseTool):
    """List tool-generated files accessible to the current user.

    Use this tool to discover files that were produced by other tools
    (``generate_document``, ``zip_tool``, CSV builders, etc.) — whether
    from the current conversation or from previous sessions.

    Filters
    -------
    - ``scope``: ``"all"`` (default) — all files the user can see; or
      ``"session"`` — restrict to the current chat session only.
    ...
    summary: ClassVar[str] = (
        "List files generated by tools that are accessible to the current user "
        "(own files and department-shared files across all sessions)"
    )
        ...
        "properties": {
            "scope": {
                ...
                "description": (
                    ...
                    "in the current chat session only. Use 'session' only when "
                    "the user explicitly asks for files from this conversation."
                ),
            },
            "tool_name": {
                "type": "string",
