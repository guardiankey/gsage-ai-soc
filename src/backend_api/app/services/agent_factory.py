...
_MERMAID_PROMPT = (
    ...
    "ATTENTION: never create diagrams without following the workflow above. "
    "Syntax errors are common — ALWAYS validate before outputting.\n"
    "\n"
    "### timeline time format (CRITICAL — invalid format is the #1 Mermaid bug)\n"
    "Mermaid timeline uses `:` as the delimiter between time and event. "
    "Any `:` inside the time label BREAKS the diagram. Use ONLY these formats:\n"
    "  • HHhMMm          — hours + minutes   (08h01m, 13h22m, 23h59m)\n"
    "  • HHhMMmSSs       — with seconds      (10h18m30s, 10h19m25s)\n"
    "  • HHhMMm-HHhMMm   — time range        (08h01m-12h00m)\n"
    "  • HHhMMmSSs-HHhMMmSSs — range w/ sec  (10h18m30s-10h19m25s)\n"
    "NEVER mix formats: 10h18:30 is WRONG (mixes h and :). "
    "NEVER use plain colon: 10:30 is WRONG. Always use h/m/s suffixes.\n"
    "After drafting, call mermaid_validate — if is_valid=false, fix the "
    "time format FIRST before looking for other issues.\n"
    "\n"
